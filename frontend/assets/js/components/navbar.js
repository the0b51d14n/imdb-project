(function () {
    ...
    if (navLinks && typeof gsap !== 'undefined') {
        ...
        const getOffsetLeft = (el) => {
            ...
            return elRect.left - navRect.left + (elRect.width - indicator.offsetWidth) / 2;
        };

        const activeBtn = navLinks.querySelector('a.active');
        document.fonts.ready.then(() => {
            if (!activeBtn) return;
            gsap.set(indicator, { x: getOffsetLeft(activeBtn) });
            gsap.to(indicator, { '--active-element-show': '1', duration: 0.2 });
        });
        ...
        navLinks.querySelectorAll('a').forEach((link, index) => {
            link.addEventListener('click', () => {
                ...
                if (!active) return;
                const oldIndex = [...navLinks.querySelectorAll('a')].indexOf(active);
                ...
                navLinks.classList.add(direction);
                active.classList.remove('active');
                link.classList.add('active');

                gsap.set(indicator, { rotateY: direction === 'before' ? '180deg' : '0deg' });

                gsap.to(indicator, {
                    keyframes: [
                        {
                            '--active-element-width': `${Math.min(spacing, navLinks.offsetWidth - 60)}px`,
                            duration: 0.3,
                            ease: 'none',
                            onStart: () => {
                                createBeamSVG(indicator);
                                gsap.to(indicator, { '--active-element-opacity': 1, duration: 0.1 });
                            },
                        },
                        {
                            ...
                            duration: 0.3,
                            onStart: () => {
                                gsap.to(indicator, { '--active-element-mask-position': '40%', duration: 0.5 });
                                gsap.to(indicator, { '--active-element-opacity': 0, delay: 0.45, duration: 0.25 });
                            },
                            onComplete: () => {
                                indicator.innerHTML = '';
                                navLinks.classList.remove('before', 'after');
                                indicator.removeAttribute('style');
                                gsap.set(indicator, {
                                    x: getOffsetLeft(link),
    ...
    requestAnimationFrame(() => {
        ...
        applyNetflixStripes('.btn-nav-register');
        applyNetflixStripes('.movie-card-overlay-btn');
    });

    function createBeamSVG(element) {
        element.innerHTML = `
        <svg viewBox="0 0 116 5" preserveAspectRatio="none" class="beam">
          <path d="M0.5 2.5L113 0.534929C114.099 0.515738 115 1.40113 115 2.5C115 3.59887 114.099 4.48426 113 4.46507L0.5 2.5Z"
                fill="url(#gradient-beam-sv)"/>
          <defs>
            <linearGradient id="gradient-beam-sv" x1="2" y1="2.5" x2="115" y2="2.5" gradientUnits="userSpaceOnUse">
              <stop stop-color="#800e13"/>
        ...
        <div class="strike">
          <svg viewBox="0 0 114 12" preserveAspectRatio="none">
            <g fill="none" stroke="#ad2831" stroke-width="0.75" stroke-linecap="round">
