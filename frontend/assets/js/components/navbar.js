(function () {
    ...
    if (burger) {
        burger.addEventListener('click', () => {
            ...
            links.style.cssText = isOpen ? '' :
                ...
                'background:rgba(13,13,13,0.98);padding:16px 24px;gap:16px;' +
                'border-bottom:1px solid #2a2a2a;z-index:99;';
        });
    }
    ...
    if (navLinks && typeof gsap !== 'undefined') {
        ...
        navLinks.appendChild(activeElement);

        const getOffsetLeft = (el) => {
            const elRect = el.getBoundingClientRect();
            const navRect = navLinks.getBoundingClientRect();
            return elRect.left - navRect.left + (elRect.width - activeElement.offsetWidth) / 2;
        };
    ...
    function applyNetflixStripes(selector, stripeCount = 40) {
        document.querySelectorAll(selector).forEach(el => {
            // Ne pas re-appliquer si déjà fait
            if (el.querySelector('.nfx-stripe')) return;
            const width = el.offsetWidth || 200;
            const step = Math.max(2, Math.floor(width / stripeCount));
    ...
    function createBeamSVG(element) {
        ...
        <svg viewBox="0 0 116 5" preserveAspectRatio="none" class="beam">
          <path d="M0.5 2.5L113 0.534929C114.099 0.515738 115 1.40113 115 2.5C115 3.59887 114.099 4.48426 113 4.46507L0.5 2.5Z" fill="url(#gradient-beam-sv)"/>
          <defs>
            <linearGradient id="gradient-beam-sv" x1="2" y1="2.5" x2="115" y2="2.5" gradientUnits="userSpaceOnUse">
              <stop stop-color="#e8500a"/>
              <stop offset="1" stop-color="#ff6520"/>
            </linearGradient>
          </defs>
        </svg>
        <div class="strike">
          <svg viewBox="0 0 114 12" preserveAspectRatio="none">
            <g fill="none" stroke="#e8500a" stroke-width="0.75" stroke-linecap="round">
